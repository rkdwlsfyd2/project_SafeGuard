...
class UnifiedComplaintManager:
    ...
    def __init__(self):
        ...
        # 사용자에게 보여줄 카테고리명
        # 사용자 요청에 따른 표시 유형 (Display Categories)
        self.AGENCY_CATEGORIES = {
            0: "경찰·검찰",      # 경찰청
            1: "교통",           # 국토교통부 (교통/도로/주택)
            2: "산업·통상",      # 고용노동부 -> 산업/경제 관련
            3: "행정·안전",      # 국방부 -> 안전/국방
            4: "행정·안전",      # 권익위 -> 행정
            5: "보건",           # 식약처
            6: "경찰·검찰",      # 대검찰청
            ...
            14: "산업·통상",     # 공정위
            15: "교육",          # 교육부
            16: "산업·통상",     # 해수부 (산업으로 분류)
            17: "산업·통상",     # 농림부 (산업으로 분류)
            18: "행정·안전",     # 소방청
            19: "행정·안전",     # 인사처
            20: "기타"
        }
    ...
    def _filter_hallucination(self, text: str) -> str:
        ...
        - 90초, 자막 제작, MBC 등 방송 자막이나 허위 텍스트 제거
        """
        # 1. 아예 무시할 고정 멘트들
        ...
        for hk in hallucination_keywords:
            if hk in text:
                logging.info(f"Hallucination detected (keyword: {hk}): {text}")
                return ""  # 환각이 포함되면 아예 빈 문자열 반환

        # 2. 너무 짧거나 의미 없는 반복 패턴 (예: "아아아아")
        if len(text) < 2: 
            return ""
